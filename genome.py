...
class Genome():
    ...
    def get_gene_spec():
        gene_spec =  {"link-shape":{"scale":1}, 
            ...
            # Advanced: Sensor genes for exceptional work
            "sensor-type":{"scale":1},  # 0=raycast, 0.33=orientation, 0.66=contact, 1=height
            "sensor-sensitivity":{"scale":2},  # How strongly sensor affects motor
            "sensor-direction-x":{"scale":1},  # Sensor direction vector
            "sensor-direction-y":{"scale":1},
            "sensor-direction-z":{"scale":1}
            }
        ind = 0
    ...
    def genome_to_links(gdicts):
        ...
        for gdict in gdicts:
            ...
            link = URDFLink(name=link_name, 
                            ...
                            sensor_type=gdict.get("sensor-type", 0),
                            sensor_sensitivity=gdict.get("sensor-sensitivity", 0),
                            sensor_direction_x=gdict.get("sensor-direction-x", 0),
                            sensor_direction_y=gdict.get("sensor-direction-y", 0),
                            sensor_direction_z=gdict.get("sensor-direction-z", 1))
            links.append(link)
...
class URDFLink:
    def __init__(self, name, parent_name, recur, 
                ...
                sensor_direction_x=0.0,
                sensor_direction_y=0.0,
                sensor_direction_z=1.0):
        self.name = name
        ...
        self.sensor_direction_x=sensor_direction_x
        self.sensor_direction_y=sensor_direction_y
        self.sensor_direction_z=sensor_direction_z
        self.sibling_ind = 1
